...
async function startRound(){return action('Starting your quote…',async()=>{const data=await request('/api/rounds',{openingClues:true});round=data.round;statistics=null;guess='';view='play';roundNumber++;rememberRound();message=`Started with ${round.revealed} ${round.revealed===1?'word':'words'} revealed (+${round.bits.toFixed(2)} bits).`;tone();});}
...
function registerTools(){
  ...
  const tools=[
    ...
    {name:'start_quote_round',description:'Start a quote with all words costing at most 10 bits already revealed and counted in the score, or its cheapest word if none qualify. Does not abandon an active round.',inputSchema:empty,execute:async()=>{if(round?.status==='playing'&&view!=='attract')throw Error('Finish the active round first.');if(!await startRound())throw Error(error);return visibleState();}},
