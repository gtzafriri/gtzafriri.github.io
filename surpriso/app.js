...
let round=null,statistics=null,view='attract',selected='subtle',guess='',busy=false,message='',error='',sound=false,audioContext,roundNumber=0;
...
async function startRound(){return action('Starting your quote…',async()=>{const data=await request('/api/rounds',{});round=data.round;statistics=null;guess='';selected='subtle';view='play';message='Words stay in their original quote order.';roundNumber++;rememberRound();});}
async function reveal(mode){
  if(!round||round.status!=='playing')throw Error('Start a round first.');
  if(!['subtle','helpful','random'].includes(mode))throw Error('Choose subtle, helpful, or random.');
  return action('Revealing your next clue…',async()=>{const data=await request(`/api/rounds/${round.id}/reveal`,{mode,expectedReveals:round.revealed});round=data.round;selected=mode;view='play';message=`Revealed: ${data.revealedWord} (+${round.lastClueBits.toFixed(2)} bits).`;tone();});
}
...
function render(focus=false){
  ...
  root.querySelector('[data-action="reveal"]')?.addEventListener('click',()=>reveal(selected));
  ...
  root.querySelectorAll('[data-clue]').forEach(button=>button.onclick=()=>{selected=button.dataset.clue;render();root.querySelector(`[data-clue="${selected}"]`).focus();});
...
function registerTools(){
  ...
  const tools=[
    ...
    {name:'start_quote_round',description:'Start a quote from the start or results screen. Does not abandon an active round.',inputSchema:empty,execute:async()=>{if(round?.status==='playing'&&view!=='attract')throw Error('Finish the active round first.');if(!await startRound())throw Error(error);return visibleState();}},
    {name:'reveal_quote_word',description:'Reveal one word with subtle, helpful, or random selection. Adds its fixed, precomputed bit cost.',inputSchema:{type:'object',properties:{mode:{type:'string',enum:['subtle','helpful','random']}},required:['mode'],additionalProperties:false},execute:async input=>{if(!await reveal(input.mode))throw Error(error);return visibleState();}},
